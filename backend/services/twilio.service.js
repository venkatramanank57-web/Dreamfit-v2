...
class TwilioWhatsAppService {
  ...
  /**
   * Send WhatsApp Template Message (for WhatsApp Business API)
   * Note: This requires Twilio Content Templates or WhatsApp Business API
   */
  async sendTemplateMessage(to, templateSid, variables = {}) {
    try {
      ...
      // Check if Twilio is configured
      if (!this.client) {
        console.log('⚠️ Twilio not configured - simulating template send');
        console.log('📨 Would send template:', templateSid);
        
        // Create a fallback text message
        let fallbackMessage = `Template ${templateSid} would be sent here.\n\n`;
        Object.entries(variables).forEach(([key, value]) => {
          fallbackMessage += `${key}: ${value}\n`;
        });
      ...
      // For Twilio Content Templates, you need to use contentSid
      // Method 1: Using Content Template (if you have created templates in Twilio Console)
      try {
        const message = await this.client.messages.create({
          from: from,
          to: toWhatsApp,
          contentSid: templateSid, // This is the Content Template SID
        ...
        // If template fails, try fallback to plain text
        console.log('⚠️ Template sending failed, trying fallback text message...');
        
        // Create a formatted text message from variables
        let fallbackText = `📱 DreamFit Notification\n\n`;
        Object.entries(variables).forEach(([key, value]) => {
          fallbackText += `${key}: ${value}\n`;
        });
      ...
      if (error.code === 63016) {
        ...
        console.log('📝 Solution: Create and approve template in Twilio Console first');
      } else if (error.code === 63017) {
        console.error('❌ Template variables mismatch');
      }
