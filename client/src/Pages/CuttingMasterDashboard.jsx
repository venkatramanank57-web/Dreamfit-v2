...
export default function CuttingMasterDashboard() {
  ...
  const [mobileTab, setMobileTab] = useState("queue"); // 'queue', 'calendar', 'tailors'
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="md:hidden bg-white border-b border-gray-200 px-4 py-3 sticky top-0 z-20">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-bold text-gray-800">Cutting Master</h1>
          </div>
          <button
        ...
        <div className="grid grid-cols-4 gap-2 mt-3 text-xs">
          <div className="text-center">
            <div className="font-bold text-blue-600">{dashboardStats?.totalWork || 0}</div>
            <div className="text-gray-500 truncate">Total</div>
          </div>
          <div className="text-center">
            <div className="font-bold text-green-600">{dashboardStats?.completedWork || 0}</div>
            <div className="text-gray-500 truncate">Done</div>
          </div>
          <div className="text-center">
            <div className="font-bold text-purple-600">{dashboardStats?.inProgressWork || 0}</div>
            <div className="text-gray-500 truncate">Progress</div>
          </div>
          <div className="text-center">
            <div className="font-bold text-red-600">{dashboardStats?.overdueWork || 0}</div>
            <div className="text-gray-500 truncate">Overdue</div>
          </div>
        </div>
        ...
        {mobileMenuOpen && (
          <div className="absolute top-16 right-4 left-4 bg-white rounded-xl shadow-xl border border-gray-200 p-2 z-30">
            <button
              onClick={() => {
                setShowOverdueOnly(!showOverdueOnly);
                setMobileMenuOpen(false);
              }}
              className="w-full text-left px-4 py-3 hover:bg-gray-50 rounded-lg text-sm"
            >
              {showOverdueOnly ? "Show All Works" : "Show Overdue Only"}
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="hidden md:block">
          ...
          {/* Tailor Performance Section - Fixed with Full Width */}
<div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <tbody>
          {tailorPerformance?.tailors?.length > 0 ? (
            tailorPerformance.tailors.map((tailor) => {
              ...
              return (
                <tr key={tailor._id} className="border-b border-gray-100 hover:bg-gray-50 transition-all">
                  <td className="py-3 px-4 whitespace-nowrap">
                    <div className="font-medium text-gray-800 text-sm">{tailor.name}</div>
                    <div className="text-xs text-gray-500">{tailor.employeeId}</div>
                  </td>
                  <td className="text-center py-3 px-4 font-bold text-sm whitespace-nowrap">{tailor.assigned || 0}</td>
                  <td className="text-center py-3 px-4 text-green-600 font-bold text-sm whitespace-nowrap">{tailor.completed || 0}</td>
                  <td className="text-center py-3 px-4 text-yellow-600 font-bold text-sm whitespace-nowrap">{tailor.pending || 0}</td>
                  <td className="text-center py-3 px-4 text-purple-600 font-bold text-sm whitespace-nowrap">{tailor.inProgress || 0}</td>
        ...
        <div className="md:hidden">
                ...
                {calendarDays.map((day, index) => (
                  <div key={index} className="aspect-square">
                    {day ? (
                      <button
                        onClick={() => handleDateSelect(day)}
                        className={`w-full h-full flex items-center justify-center rounded-lg text-xs transition-all relative
                          ${selectedDate === day.dateStr 
              ...
              <div className="flex gap-2 mb-3">
                <button
                  ...
                  className={`flex-1 py-2 text-xs rounded-lg transition ${
                    viewMode === "today" ? "bg-purple-600 text-white" : "bg-gray-100 text-gray-600"
                  }`}
                >
                  Today
                </button>
                <button
                  onClick={() => handleViewModeChange("tomorrow")}
                  className={`flex-1 py-2 text-xs rounded-lg transition ${
                    viewMode === "tomorrow" ? "bg-purple-600 text-white" : "bg-gray-100 text-gray-600"
                  }`}
                >
                  Tomorrow
                </button>
              ...
              <div className="space-y-3 max-h-[500px] overflow-y-auto">
                {workQueue?.works?.length > 0 ? (
                  workQueue.works
                    ...
                    .slice(0, 10)
                    .map((work) => {
                      const priority = getWorkPriority(work, 'mobile');
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between mb-1">
                            ...
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${getStatusColor(work.status)}`}>
                              {getStatusBadge(work.status)}
                            </span>
                          </div>
                          ...
                          {work.estimatedDelivery && (
                            <div className="mt-2 text-[10px]">
                              <span className="text-gray-500">Due: </span>
                              <span className={new Date(work.estimatedDelivery) < new Date() && work.status !== 'ready-to-deliver' ? 'text-red-600 font-bold' : 'text-gray-700'}>
                                {new Date(work.estimatedDelivery).toLocaleDateString()}
                              </span>
                            </div>
                          )}
          ...
          {mobileTab === "tailors" && (
            <div className="bg-white rounded-xl shadow-sm p-4">
              ...
              <div className="space-y-3">
                {tailorPerformance?.tailors?.length > 0 ? (
                  tailorPerformance.tailors.slice(0, 5).map((tailor) => {
                    const isAvailable = availableTailors?.availableTailors?.some(t => t._id === tailor._id);
                    
                    return (
                      <div key={tailor._id} className="border border-gray-200 rounded-lg p-3">
                        <div className="flex items-center justify-between mb-2">
                          <div>
                            <div className="font-medium text-gray-800">{tailor.name}</div>
                            <div className="text-xs text-gray-500">{tailor.employeeId}</div>
                          ...
                          {isAvailable ? (
                            ...
                            <span className="px-2 py-1 bg-gray-100 text-gray-600 rounded-full text-[10px]">
                              Busy
                            </span>
                          )}
                        </div>
                        ...
                        <div className="grid grid-cols-4 gap-1 text-center text-xs mt-2">
                          <div>
                            <div className="font-bold">{tailor.assigned || 0}</div>
                            <div className="text-gray-500">Assigned</div>
                          </div>
                          <div>
                            <div className="font-bold text-green-600">{tailor.completed || 0}</div>
                            <div className="text-gray-500">Done</div>
                          </div>
                          <div>
                            <div className="font-bold text-yellow-600">{tailor.pending || 0}</div>
                            <div className="text-gray-500">Pending</div>
                          </div>
                          <div>
                            <div className="font-bold text-purple-600">{tailor.inProgress || 0}</div>
                        ...
                        <button
                          onClick={() => handleViewTailor(tailor._id)}
                          className="w-full mt-3 text-xs px-3 py-2 bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-100 transition"
                        >
                          View Profile
                        </button>
                      </div>
                  ...
                  <p className="text-center py-8 text-gray-500">No tailors found</p>
                )}
              </div>
            </div>
          )}
