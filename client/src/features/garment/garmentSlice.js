...
// export const fetchCustomerOrderDates = createAsyncThunk(
//   "garment/fetchCustomerOrderDates",
//   async ({ month, year }, { rejectWithValue }) => {  // 🔥 No customerId needed
//     try {
//       if (!month || !year) {
//         return rejectWithValue("Month and year are required");
//       }
      
//       console.log(`🟢 Fetching all delivery dates: month: ${month}, year: ${year}`);
      
//       const response = await garmentApi.getAllDeliveryDates({ month, year }); // 🔥 New API call
      
//       return {
//         allDates: response.allDates || [],  // 🔥 All dates with counts
//         month,
//         year
//       };
//     } catch (error) {
//       console.error('❌ fetchAllDeliveryDates error:', error);
...
// ============================================
// 🟢 NEW: Get ALL delivery dates (all customers)
// ============================================
export const fetchAllDeliveryDates = createAsyncThunk(
  "garment/fetchAllDeliveryDates",
  async ({ month, year }, { rejectWithValue }) => {
    try {
      ...
      const response = await garmentApi.getAllDeliveryDates({ month, year });
      
      return {
        allDates: response.allDates || [],
        month,
        year
      };
    } catch (error) {
...
const initialState = {
  ...
  // 📅 UPDATED: Calendar state for all deliveries
  calendar: {
    allDeliveryDates: [],  // 🔥 Changed from customerOrderDates
    loading: false,
    month: null,
    year: null
  }
...
const garmentSlice = createSlice({
  ...
  reducers: {
    ...
    // 📅 UPDATED: Calendar reducers
    clearCalendarData: (state) => {
      state.calendar = {
        allDeliveryDates: [],  // 🔥 Changed
        loading: false,
        month: null,
        year: null
      };
    ...
    setCalendarMonth: (state, action) => {
      const { month, year } = action.payload;  // 🔥 Removed customerId
      state.calendar.month = month;
      state.calendar.year = year;
    }
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAllDeliveryDates.pending, (state) => {
        state.calendar.loading = true;
        state.error = null;
        console.log('⏳ fetchAllDeliveryDates pending');
      })
      .addCase(fetchAllDeliveryDates.fulfilled, (state, action) => {
        state.calendar.loading = false;
        console.log('✅ fetchAllDeliveryDates fulfilled - dates:', action.payload.allDates?.length);
        
        state.calendar.allDeliveryDates = action.payload.allDates || [];
        state.calendar.month = action.payload.month;
        state.calendar.year = action.payload.year;
      })
      .addCase(fetchAllDeliveryDates.rejected, (state, action) => {
        state.calendar.loading = false;
        state.error = action.payload;
        state.calendar.allDeliveryDates = [];
        console.error('❌ fetchAllDeliveryDates rejected:', action.payload);
      });
  },
});
...
export const selectAllDeliveryDates = (state) => {
  const garmentState = getGarmentState(state);
  const dates = garmentState.calendar?.allDeliveryDates || [];
  console.log('🔍 [Selector] selectAllDeliveryDates:', dates.length);
  return dates;
};
