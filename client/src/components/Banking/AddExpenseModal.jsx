...
// components/Banking/AddExpenseModal.jsx - COMPLETE FIXED VERSION
import React, { useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import {
  X,
...
const EXPENSE_CATEGORIES = [
  ...
  { value: 'other-expense', label: 'Other Expense', icon: <Plus size={24} />, color: 'gray' } // ✅ Added back Other Expense
];
...
export default function AddExpenseModal({ onClose, accountType = null }) {
  const dispatch = useDispatch();
  const { loading } = useSelector((state) => state.transaction);
  const [isSubmitting, setIsSubmitting] = useState(false); // ✅ Added for extra safety
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("📤 Sending expense transaction with date:", {
      ...
      const transactionData = {
        ...
        transactionDate: selectedDate.toISOString(), // ✅ Send full ISO string
          ...
          <div className="flex gap-3 pt-4">
            ...
            {step === 3 ? (
              <button
                type="submit"
                disabled={loading || isSubmitting}
                className={`flex-1 py-3 bg-red-600 text-white rounded-xl font-medium transition-all ${
                  (loading || isSubmitting) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-700'
                }`}
              >
